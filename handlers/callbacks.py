...
"""
        ...
        if pro_models:
            ...
            for model in pro_models:
                ...
                if current_text_model == model:
                    pro_buttons.append(
                        InlineKeyboardButton(f"✅ {model_short[:18]}", callback_data=f"set_text_model_{model}")
                    )
                else:
                    pro_buttons.append(
                        InlineKeyboardButton(f"🔥 {model_short[:18]}", callback_data=f"set_text_model_{model}")
        ...
        if current_text_model == "auto":
            keyboard.append(
                [InlineKeyboardButton("✅ Автоматический выбор (текст)", callback_data="set_text_model_auto")]
            )
        else:
            keyboard.append(
                [InlineKeyboardButton("🔄 Автоматический выбор (текст)", callback_data="set_text_model_auto")]
            ...
            for model in premium_models:
                ...
                if current_image_model == model:
                    premium_buttons.append(
                        InlineKeyboardButton(f"✅ {model_short[:18]}", callback_data=f"set_image_model_{model}")
                    )
                else:
                    premium_buttons.append(
                        InlineKeyboardButton(f"🔴 {model_short[:18]}", callback_data=f"set_image_model_{model}")
        ...
        if current_image_model == "auto":
            keyboard.append(
                [InlineKeyboardButton("✅ Автоматический выбор (изображения)", callback_data="set_image_model_auto")]
            )
        else:
            keyboard.append(
                [InlineKeyboardButton("🔄 Автоматический выбор (изображения)", callback_data="set_image_model_auto")]
...
"""
    ...
    # Установка текстовой модели
        ...
        await safe_callback_answer(
            query, f"✅ Текстовая модель установлена: {model_key if model_key != 'auto' else 'Автоматический выбор'}"
        )
    ...
    # Установка модели изображений
        ...
        await safe_callback_answer(
            query, f"✅ Модель изображений установлена: {model_key if model_key != 'auto' else 'Автоматический выбор'}"
        )
...
"""
    ...
    # Обработка избранного
        ...
        await db.add_favorite(
            user_id=user_id, content=original_text, content_type="image" if query.message.photo else "text"
        )
