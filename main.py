...
from handlers.admin import broadcast_command, health_command, logs_command, users_command
...
def main():
    ...
    # Обязательные env (TELEGRAM_BOT_TOKEN, ARTEMOX_API_KEY) проверены при импорте config — при отсутствии процесс завершается с понятным сообщением
    ...
    if use_webhooks and webhook_url:
        ...
        application.run_polling(
            allowed_updates=Update.ALL_TYPES, drop_pending_updates=True, timeout=5, poll_interval=0
        )
