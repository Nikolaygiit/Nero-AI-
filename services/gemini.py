...
class GeminiService:
    ...
    async def generate_with_image_context(
        self, prompt: str, image_base64: str, user_id: Optional[int] = None, use_context: bool = True
    ) -> str:
        """
        ...
        available_models = await self.list_available_models()
        vision_models = [
            m for m in available_models if any(x in m.lower() for x in ["flash", "pro", "1.5", "2.0", "2.5"])
    ...
    async def analyze_image(
        ...
        vision_models = [
            m for m in available_models if any(x in m.lower() for x in ["flash", "pro", "1.5", "2.0", "2.5", "3.0"])
        ]
