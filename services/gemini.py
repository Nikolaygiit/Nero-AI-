...
class GeminiService:
    ...
    async def _prepare_messages_context(
        self, prompt: str, user_id: Optional[int], use_context: bool, rag_context: Optional[str]
    ) -> List[Dict[str, Any]]:
        """Prepare context and messages for the LLM"""
        context_messages = []
    ...
    async def _execute_legacy_fallback(
        ...
        """Execute legacy fallback loop using httpx"""
    ...
    async def generate_content(
        ...
        """
        # 1. Prepare messages and context
        ...
        return await self._execute_legacy_fallback(messages, models_to_try, prompt, user_id)
