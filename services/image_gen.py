...
class ImageGeneratorStrategy(ABC):
    ...
    @abstractmethod
    async def generate(
        self, prompt: str, style: Optional[str] = None, size: Optional[str] = None
    ) -> bytes:
...
class ArtemoxImageGenerator(ImageGeneratorStrategy):
    ...
    async def _attempt_generation(
        ...
        """
        Попытка генерации изображения одной моделью.
        Возвращает байты изображения или вызывает исключение при ошибке.
        """
        data = {
            "model": model_name,
            "prompt": prompt,
        ...
        # Если статус не 200 или формат ответа некорректный
        if response.status_code != 200:
            response.raise_for_status()

        # Если статус 200, но мы здесь -> значит формат не подошел
        raise ValueError("Некорректный формат ответа или пустой результат")
            ...
            for model_name in models_to_try:
                try:
                    return await self._attempt_generation(
                        client, model_name, prompt, image_size, url, headers
                    )
                    ...
                    # Ошибка формата ответа (status 200)
                    error_msg = str(e)
                    last_error = error_msg
                    logger.warning(f"Ошибка 200 для {model_name}: {error_msg}")
                    continue
...
async def generate_with_queue(
    prompt: str, user_id: int, style: Optional[str] = None, size: Optional[str] = None
) -> Tuple[bytes, str]:
    """
