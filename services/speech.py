...
import config
from services.llm_common import build_headers
...
async def speech_to_text(voice_bytes: bytes, lang: str = "ru") -> Optional[str]:
    ...
    # Только Authorization: для multipart/form httpx сам выставит Content-Type
...
async def text_to_speech(text: str, lang: str = "ru") -> Optional[bytes]:
    ...
    """
    url = f"{config.GEMINI_API_BASE.rstrip('/')}/audio/speech"
    headers = build_headers(config.GEMINI_API_KEY)
